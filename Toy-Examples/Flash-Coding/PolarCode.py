...
class PolarCode():
    ...
    def encoder(self, msg, frozenValues, t):
        ...
        # =====================================  Decoders =========================== #
    def decodeSC(self, y, frozenValues):
        # --- Initiazations
        L = np.zeros((self.N + 1, self.n))  # beliefs
        stateVec = np.zeros((2 * self.n - 1))  # State vector
        L[0, :] = y  # belief for root
        node, depth, done = 0, 0, 0
        uHat = 2 * np.ones((self.N + 1, self.n))
        ...
        while (done == 0):  # Traversal loop
            ...
            if (depth == self.N):
                ...
                if (node in self.frozenPos):
                    uHat[self.N, node] = frozenValues[np.where(self.frozenPos == node)]
                    self.LLRs[node] = frozenValues[np.where(self.frozenPos == node)]
                else:
                    # Hard threshold
                    self.LLRs[node] = L[self.N, node]
                    if (L[self.N, node] >= 0):
                        uHat[self.N, node] = 0
                    else:
                        uHat[self.N, node] = 1
            ...
            # ==================== Other node  ==================== #
                ...
                if (stateVec[npos] == 0):
                    ...
                    # --- MinSum
                    L[depth, temp * node:temp * (node + 1)] = minSum(a, b)
                    ...
                    if (stateVec[npos] == 1):
                        ...
                        dL = int(depth + 1)
                        tempL = int(temp / 2)
                        uHatn = uHat[dL, tempL * nodeL:tempL * (nodeL + 1)]
                        ...
                        # --- Save Beliefs
                        L[depth, temp * node:temp * (node + 1)] = rept
                    ...
                    # ==================== U Step  ==================== #
                        ...
                        uHatL = uHat[dC, tempC * nodeL:tempC * (nodeL + 1)]

                        # --- Incoming decisions from the right child
                        uHatR = uHat[dC, tempC * nodeR:tempC * (nodeR + 1)]

                        if (len(uHatL) != len(uHatR)):
                            print('Hello')
                        # --- Combine
                        uHat[depth, temp * node:temp * (node + 1)] = np.concatenate([((uHatL + uHatR) % 2), uHatR])

                        # --- Go back to parent
                        node = int(np.floor(node / 2))
                        depth = int(depth - 1)
        return uHat[self.N, self.msgPos], self.LLRs[self.msgPos]
    # =====================================  Decoder =========================== #
